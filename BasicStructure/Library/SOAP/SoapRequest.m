...
- (void) send {
	...
	NSString *strCookie = [[NSUserDefaults standardUserDefaults] objectForKey:@"cstcookie"];
	NSLog(@"Getting cookie -->%@",strCookie);
	if(strCookie)
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response {
	...
	NSLog(@"All header fields->%@",[(NSHTTPURLResponse*)response allHeaderFields]);

	NSHTTPURLResponse *HTTPResponse = (NSHTTPURLResponse *)response;
    NSDictionary *fields = [HTTPResponse allHeaderFields];
    NSString *cookie = [fields valueForKey:@"Set-Cookie"];
	
	NSArray *arrCookies = [cookie componentsSeparatedByString:@","];
	
	NSString *rlCookie=nil;
	if([arrCookies count]>1)
		rlCookie = [arrCookies objectAtIndex:1];
	
	NSLog(@"new Cookie ->%@",rlCookie);
	
	[[NSUserDefaults standardUserDefaults] setObject:rlCookie forKey:@"cstcookie"];
