...
+ (NSString*) getNodeValue: (CXMLNode*) node withName: (NSString*) name {
	...
	if([node isKindOfClass: [CXMLElement class]])
	{
		...
		if(child != nil) {
//            NSLog(@"name->%@ and Childvalue->%@",name,[[child stringValue] trimWhiteSpace]);
			return [[child stringValue] trimWhiteSpace];
		}
	}
	...
	if(child != nil) {
//        NSLog(@"name1->%@ and Childvalue1->%@",name,[[child stringValue] trimWhiteSpace]);
		return [[child stringValue] trimWhiteSpace];
	}
	return nil;
}
